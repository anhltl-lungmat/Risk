...
function mapMatchLevel(m) {
  return {
    ...
    TRUNG_KHOP_1_NHOM: "Trùng khớp 1 phân nhóm",
...
function CCDVBadge({ status, size = "md", fullWidth }) {
  ...
  if (status === "KHONG_DUOC_CCDV") {
    return (
      <span className={`${w} items-center gap-1.5 ${s} font-bold text-rose-700 bg-rose-50 border border-rose-200 rounded`}>
  ...
  return (
    <span className={`${w} items-center gap-1.5 ${s} font-bold text-emerald-700 bg-emerald-50 border border-emerald-200 rounded`}>
...
function RiskLevelChip({ level, score, size = "md" }) {
  const cfg = {
    CAO: { text: "text-rose-700" },
    TRUNG_BINH: { text: "text-amber-700" },
    THAP: { text: "text-emerald-700" }
  }[level] || { text: "text-slate-700" };
  const s = size === "sm" ? "text-[11px]" : "text-[12px]";
  return (
    <span className={`inline-flex items-center gap-1 ${s} font-bold ${cfg.text}`}>
      {mapLevel(level)}{score != null ? <span className="font-mono opacity-70">· {score}</span> : null}
    </span>);
...
function RiskGauge({ score, level, style = "bar" }) {
  ...
  const cfg = {
    CAO: { fill: "bg-rose-500", text: "text-rose-700", track: "bg-rose-100" },
    TRUNG_BINH: { fill: "bg-amber-500", text: "text-amber-700", track: "bg-amber-100" },
    THAP: { fill: "bg-emerald-500", text: "text-emerald-700", track: "bg-emerald-100" }
  }[level] || { fill: "bg-slate-400", text: "text-slate-700", track: "bg-slate-100" };
    ...
    return (
      <div className="flex items-center gap-2">
        <div className="flex gap-0.5">
          ...
          <span key={i} className={`w-1 h-3 rounded-sm ${i < filled ? cfg.fill : "bg-slate-150"}`} />
          )}
        </div>
        <span className={`font-mono text-xs font-bold ${cfg.text} tabular-nums`}>{score}</span>
      </div>);
  ...
  return (
    <div className="flex items-center gap-2 min-w-[100px]">
      <div className={`flex-1 h-1.5 rounded-full ${cfg.track} overflow-hidden`}>
        <div className={`h-full ${cfg.fill} transition-all duration-500`} style={{ width: `${pct}%` }}></div>
      </div>
      <span className={`font-mono text-[12px] font-bold ${cfg.text} tabular-nums w-7 text-right`}>{score}</span>
    </div>);
...
function MatchLevelPill({ match }) {
  const cfg = {
    TRUNG_KHOP_TOAN_BO: { bg: "bg-rose-50", text: "text-rose-700", border: "border-rose-200" },
    TRUNG_KHOP_1_NHOM: { bg: "bg-amber-50", text: "text-amber-700", border: "border-amber-200" },
    KHONG_TRUNG_KHOP: { bg: "bg-slate-50", text: "text-slate-500", border: "border-slate-200" }
  }[match] || { bg: "bg-slate-50", text: "text-slate-500", border: "border-slate-200" };
  return (
...
Object.assign(window, {
  mapEntity, mapEntityFull, mapSource, mapMatchLevel, mapAction, mapLevel,
  CCDVBadge, BlacklistBadge, RiskLevelChip, RiskGauge, MatchLevelPill,
